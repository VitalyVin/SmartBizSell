 ...
 * - <h1> -> разделитель из "=" (60 символов)
 * - <h2> -> разделитель из "-" (60 символов)
 * - <h3> -> разделитель из "." (40 символов)
 * - <p> -> двойные переносы строк
 * - <br> -> одинарные переносы строк
 * - <li> -> маркеры списка "•" с отступом
 * 
...
function convertHtmlToTxt(string $html): string
{
    ...
    // Удаляем HTML теги, но сохраняем структуру через замену тегов на текстовые разделители
    $text = $html;
    
    // Заменяем заголовки на текстовые эквиваленты с разделителями
    // H1: двойная линия из "=" (60 символов) сверху и снизу
    $text = preg_replace('/<h1[^>]*>(.*?)<\/h1>/is', "\n\n" . str_repeat('=', 60) . "\n$1\n" . str_repeat('=', 60) . "\n", $text);
    // H2: двойная линия из "-" (60 символов) сверху и снизу
    $text = preg_replace('/<h2[^>]*>(.*?)<\/h2>/is', "\n\n" . str_repeat('-', 60) . "\n$1\n" . str_repeat('-', 60) . "\n", $text);
    // H3: линия из "." (40 символов) снизу
    ...
    // Заменяем списки: удаляем теги ul/ol, преобразуем li в маркеры
    $text = preg_replace('/<ul[^>]*>/i', "\n", $text);
    $text = preg_replace('/<\/ul>/i', "\n", $text);
    $text = preg_replace('/<ol[^>]*>/i', "\n", $text);
    $text = preg_replace('/<\/ol>/i', "\n", $text);
    // Элементы списка с отступом и маркером "•"
    ...
    // Три и более переноса строк подряд -> два переноса
    $text = preg_replace('/\n{3,}/', "\n\n", $text);
    $text = trim($text);
 ...
 * 
 * Добавляет к тексту:
 * - Заголовок документа (TERM SHEET) с разделителями
 * - Основной контент
 * - Футер с информацией о дате создания
 * 
 * @param string $text Исходный текст Term Sheet
 * @return string Отформатированный текст с заголовком и футером
 */
function formatPlainText(string $text): string
{
    // Добавляем заголовок документа с разделителями из "=" (70 символов)
    ...
    // Добавляем футер с разделителями из "-" (70 символов)
    $output .= "\n\n" . str_repeat('-', 70) . "\n";
