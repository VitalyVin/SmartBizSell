...
- PHPMailer (для отправки email) - опционально, но рекомендуется
...
### 4. Миграция: тип компании (стартап / зрелая)

В таблицу `seller_forms` добавляется колонка `company_type` (ENUM 'startup', 'mature') для разделения анкет стартапов и зрелых компаний.

**Через phpMyAdmin:** откройте вкладку SQL, вставьте содержимое файла `db/migration_add_company_type.sql`, нажмите «Вперед».  
**Или через импорт:** Импорт → выберите `db/migration_add_company_type.sql` → Вперед.

### 5. Миграция: кастомное название карточки тизера

В таблицу `published_teasers` добавляется колонка `card_title` для отображения на главной странице названия, заданного модератором.

**Через phpMyAdmin:** SQL → вставьте содержимое `db/migration_add_card_title.sql` → Вперед.  
**Или через импорт:** выберите файл `db/migration_add_card_title.sql`.

### 6. Миграция: счётчик просмотров карточек

В таблицу `published_teasers` добавляется колонка `views` для отслеживания количества просмотров карточки (открытий модального окна). При повторной публикации новой версии тизера просмотры автоматически переносятся на новую версию.

**Через phpMyAdmin:** SQL → вставьте содержимое `db/migration_teaser_views.sql` → Вперед.  
**Или через импорт:** выберите файл `db/migration_teaser_views.sql`.

### 7. Миграция для документов активов
...
### 8. Миграция для восстановления пароля

Для работы функции восстановления пароля необходимо создать таблицу токенов:
...
2. Выберите вашу базу данных
3. Перейдите на вкладку **"SQL"**
4. Откройте файл `db/migration_password_reset.sql` в текстовом редакторе
5. Скопируйте содержимое файла
6. Вставьте SQL-код в окно phpMyAdmin
7. Нажмите **"Вперед"** (Execute)
...
1. В phpMyAdmin выберите вашу базу данных
2. Перейдите на вкладку **"Импорт"**
3. Нажмите **"Выбрать файл"** и выберите `db/migration_password_reset.sql`
4. Нажмите **"Вперед"**

После успешного импорта будет создана таблица:
- `password_reset_tokens` - токены для восстановления пароля

**Примечание:** Таблица также может быть создана автоматически при первом использовании функционала восстановления пароля (через функцию `ensurePasswordResetTokensTable()`), но рекомендуется создать её вручную для контроля.

### 9. Миграция для блога

Для раздела блога создаётся таблица `blog_posts`. Подробные шаги выполнения — в [MIGRATION_INSTRUCTIONS.md](MIGRATION_INSTRUCTIONS.md).

**Кратко:** выполните в phpMyAdmin (SQL или Импорт) файл `db/migration_blog.sql`. Или откройте в браузере `run_migration_blog.php` (нужна авторизация модератора).

### 10. Установка PHPMailer (для отправки email)

Для работы функции восстановления пароля рекомендуется установить PHPMailer.
...
1. Скачайте PHPMailer с GitHub: https://github.com/PHPMailer/PHPMailer/releases
2. Распакуйте архив в папку `vendor/PHPMailer/PHPMailer/` в корне проекта
3. Убедитесь, что файл `vendor/PHPMailer/PHPMailer/src/PHPMailer.php` существует

**Примечание:** Если PHPMailer не установлен, система будет использовать встроенную функцию `mail()`, но это менее надежно для SMTP.

### 11. Настройка папки для загрузки документов
...
define('TOGETHER_MODEL', 'meta-llama/Llama-3.3-70B-Instruct-Turbo'); // Основная модель ИИ
```
...
├── api/
│   └── estimate_valuation.php    # API оценки (POST, JSON)
├── login.php, register.php, logout.php
├── forgot_password.php            # Восстановление пароля (запрос)
├── reset_password.php             # Восстановление пароля (установка)
├── dashboard.php                  # Личный кабинет продавца
├── seller_form.php, view_form.php
├── generate_teaser.php            # Генерация тизера через ИИ
├── calculate_multiplier_valuation.php  # Расчет оценки по мультипликаторам
├── save_price_data.php            # Сохранение данных о цене
├── save_teaser_edits.php          # Сохранение правок тизера
├── moderation.php, moderation_api.php
├── submit_teaser_moderation.php, view_teaser.php
├── term_sheet_form.php, generate_term_sheet.php
├── investor_utils.php
├── blog.php, blog_post.php, blog_api.php
├── run_migration_blog.php         # Выполнение миграции блога
├── upload_asset_document.php, delete_asset_document.php
├── get_asset_documents.php, download_asset_document.php
├── script.js, styles.css, .htaccess
└── db/
    ├── schema.sql, install.sql
    ├── migration_published_teasers.sql
    ├── migration_add_company_type.sql, migration_add_card_title.sql
    ├── migration_asset_documents.sql, migration_password_reset.sql
    ├── migration_blog.sql
    └── README.md
```
...
- `index.php` - Главная страница с каталогом бизнесов
- `estimate.php` - Быстрая оценка бизнеса (без входа)
- `api/estimate_valuation.php` - API оценки (POST, JSON)
- `api/increment_teaser_views.php` - API увеличения счётчика просмотров карточки
- `register.php` - Страница регистрации
- `login.php` - Страница входа
- `logout.php` - Выход из системы
- `forgot_password.php`, `reset_password.php` - Восстановление пароля
- `dashboard.php` - Личный кабинет продавца
...
- `db/install.sql` - Готовый SQL для импорта
- `db/migration_published_teasers.sql` - Модерация и публикация тизеров
- `db/migration_add_company_type.sql` - Тип компании (стартап/зрелая)
- `db/migration_add_card_title.sql` - Кастомное название карточки тизера
- `db/migration_teaser_views.sql` - Счётчик просмотров карточек
- `db/migration_asset_documents.sql` - Документы активов
- `db/migration_password_reset.sql` - Восстановление пароля
- `db/migration_blog.sql` - Блог (таблица blog_posts)
- `db/README.md` - Дополнительная документация по БД (полный список миграций)
...
4. **Выбор типа компании** (стартап / зрелая) при создании анкеты; отдельные поля и логика тизеров для стартапов
5. **Анкеты** - заполнение и сохранение анкет в БД (с поддержкой черновиков)
6. **Отслеживание статуса** - просмотр статуса анкет:
   - Черновик
   - Отправлена
   - На проверке
   - Одобрена
   - Отклонена
7. **Генерация тизеров** - автоматическое создание тизеров через ИИ
...
11. **Оценка по мультипликаторам** — расчёт стоимости бизнеса в личном кабинете
...
2. **Бейдж «Стартап»** — на карточках в каталоге отображается значок для объявлений с типом «стартап»
3. **Счётчик просмотров** — при открытии карточки отображается количество просмотров (аналогично статьям в блоге)
4. **Фильтрация** - поиск по отрасли, цене, городу
5. **Детальная информация** - полное описание бизнеса с финансовыми показателями
6. **Финансовые графики** - визуализация финансовых данных в тизерах
7. **Современные карточки** - красивое представление бизнесов с чипами и статистикой
8. **Скачивание документов** - доступ к документам, загруженным продавцом (презентации, отчеты, фотографии и т.д.)

### Быстрая оценка без регистрации:

- Страница **estimate.php**: ввод минимальных данных (описание деятельности, выручка, EBITDA), расчёт диапазона оценки по DCF и мультипликаторам; кнопка «Оценить бизнес» в hero на главной.
...
6. Для восстановления пароля проверьте настройки SMTP в `config.php`:
   - `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`
   - Убедитесь, что PHPMailer установлен (или используется `mail()`)
